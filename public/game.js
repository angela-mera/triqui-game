class Game {
    constructor() {
        ...
        this.newSeriesBtn = document.getElementById('new-series-btn');
        ...
        this.restartBtn.textContent = 'Abandonar partida';
        this.round = 1;
        this.maxRounds = 5;
        this.scores = { X: 0, O: 0 };
        this.points = { X: 0, O: 0 };
                ...
                btn.addEventListener('click', () => {
                    ...
                    if (this.gameId) {
                        this.socket.emit('symbolColorChange', { 
                            gameId: this.gameId, 
                            color: color,
                            symbol: this.playerSymbol // Enviar el símbolo del jugador
                        });
                    }
                });
            });
    ...
    setupEventListeners() {
        ...
        // New series button event listener
        if (this.newSeriesBtn) {
    ...
    setupSocketListeners() {
        ...
        this.socket.on('gameState', ({ board, currentPlayer, scores, winner, isDraw, round, seriesWinner }) => {
            ...
            if (seriesWinner) {
                ...
                this.board.style.pointerEvents = 'none';
                this.restartBtn.textContent = 'Reiniciar serie';
                ...
                this.board.style.pointerEvents = 'none';
                this.restartBtn.textContent = 'Siguiente partida';
                this.restartBtn.style.display = 'block';
            } else if (isDraw) {
        ...
        this.socket.on('opponentDisconnected', () => {
            ...
            this.status.textContent = 'Opponent disconnected! Waiting for reconnection...';
            this.board.style.pointerEvents = 'none';
        });

        this.socket.on('opponentReconnected', () => {
            console.log('Opponent reconnected');
            this.status.textContent = 'Opponent reconnected!';
            this.board.style.pointerEvents = this.playerSymbol === this.currentPlayer ? 'auto' : 'none';
        });

        this.socket.on('opponentTimeout', () => {
            console.log('Opponent timeout');
            this.status.textContent = 'Opponent did not reconnect. You can restart the game.';
            this.restartBtn.style.display = 'block';
        });
        ...
        this.socket.on('opponentAbandoned', () => {
            ...
            this.board.style.pointerEvents = 'none';
            this.restartBtn.textContent = 'Nueva partida';
            this.restartBtn.style.display = 'block';
        });
    ...
    updateBoard(board) {
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            if (board[i]) {
                ...
                // Aplicar el color correspondiente según el símbolo
                if (board[i] === this.playerSymbol) {
                    cell.style.color = getComputedStyle(document.documentElement).getPropertyValue('--symbol-color');
                } else {
                    cell.style.color = getComputedStyle(document.documentElement).getPropertyValue('--opponent-symbol-color');
                }
            } else {
                cell.style.color = '';
            }
    ...
    updateBoardDisplay() {
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            if (this.boardState[i]) {
                ...
                // Aplicar el color correspondiente según el símbolo
                if (this.boardState[i] === this.playerSymbol) {
                    cells[i].style.color = getComputedStyle(document.documentElement).getPropertyValue('--symbol-color');
                } else {
                    cells[i].style.color = getComputedStyle(document.documentElement).getPropertyValue('--opponent-symbol-color');
                }
            } else {
                cells[i].style.color = '';
            }
